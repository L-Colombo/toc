...
def admits_float(
    expect: Optional[float], actual: Optional[float], tolerance: float
) -> bool:
...
class FontFilter:
    ...
    def __init__(self, font_dict: dict):
        ...
        # x * False = 0
        self.flags = (
            0b00001 * font_dict.get("superscript", False)
        ...
        self.ign_mask = (
            0b00001 * ("superscript" in font_dict)
...
class BoundingBoxFilter:
    ...
    def admits(self, spn: dict) -> bool:
        ...
        return (
            admits_float(self.left, bbox[0], self.tolerance)
            and admits_float(self.top, bbox[1], self.tolerance)
            and admits_float(self.right, bbox[2], self.tolerance)
            and admits_float(self.bottom, bbox[3], self.tolerance)
        )
...
class Recipe:
    ...
    def extract_block(self, block: dict, page: int) -> List[ToCEntry]:
        ...
        try:
            ...
            return [
                ToCEntry(level, title, page, vpos) for level, title in titles.items()
            ]
...
def extract_toc(doc: Document, recipe: Recipe) -> List[ToCEntry]:
    ...
    for page in doc.pages():
        for blk in page.get_textpage().extractDICT().get("blocks", []):
            result.extend(recipe.extract_block(blk, page.number + 1))
